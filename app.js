...
function sortResults(criterion) {
  ...
  if (criterion === 'most-popular') {
    results = results.sort((a, b) => (+b.favorite_count + +b.retweet_count) - (+a.favorite_count + +a.retweet_count));
    renderResults();
  }
  ...
  if (criterion === 'most-popular-browse') {
    browseDocuments = browseDocuments.sort((a, b) => (+b.favorite_count + +b.retweet_count) - (+a.favorite_count + +a.retweet_count));
    renderBrowse();
  }
}
...
function renderResults() {
  ...
  `.replace(/\.\.\/\.\.\/tweets_media\//g, 'MusicoTeorico/tweets_media/'));
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  if (query) {
    ...
    searchTab(); // activa la pestaña de búsqueda
  }
});
